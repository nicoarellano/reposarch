const map = new maplibregl.Map({
  container: 'map',
  style:
    'https://api.maptiler.com/maps/backdrop/style.json?key=v73CsWuZ4JslNyKg3ZEH',
...
function filterBy(month) {
  ...
  // Update active abbreviated month label under the slider
  const monthLabels = document.querySelectorAll('.month-label');
  monthLabels.forEach((label, index) => {
    label.classList.toggle('active', index === month);
  });
}
